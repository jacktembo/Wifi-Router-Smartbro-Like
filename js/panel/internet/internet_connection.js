...
(function($) {
    ...
    $.fn.objInternetConn = function(InIt) {
        ...
        this.getCellularPostData = function() {
            ...
            if (NW_modeSelected != pre_NW_mode) { //改变主模式，则设置优先选择模式
...
function pPDPdefChange() {
    ...
    if (document.getElementById("pPDPdef_chk").checked)
        $("#s_PDP_default :input").removeAttr('disabled');
    else {
        $("#s_PDP_default :input").attr('disabled', true);
        $("#s_PDP_default :input").attr('checked', false);
    }
}

function pPDPdedChange() {
    if (document.getElementById("pPDPded_chk").checked) {
        $("#s1_PDP_dedicated :input").removeAttr('disabled');
        $("#s2_PDP_dedicated :input").removeAttr('disabled');
        $("#s3_PDP_dedicated :input").removeAttr('disabled');
    } else {
        $("#s1_PDP_dedicated :input").attr('disabled', true);
        $("#s2_PDP_dedicated :input").attr('disabled', true);
        $("#s3_PDP_dedicated :input").attr('disabled', true);
    }
}
